...
const stockController = {
    ...
    try {
      ...
      if (!productId || !warehouseId || !quantity) {
        return res.status(400).json({ 
          error: 'productId, warehouseId và quantity là bắt buộc' 
        });
      }

      if (quantity <= 0) {
        return res.status(400).json({ 
          error: 'Quantity phải lớn hơn 0' 
        });
      }
      
      if (type && !['IN', 'OUT'].includes(type)) {
        return res.status(400).json({ 
          error: 'Type phải là IN hoặc OUT' 
        });
      }
      ...
      if (error.message === 'Warehouse is not active') {
        return res.status(400).json({ error: 'Kho không hoạt động' });
      }
      if (error.message === 'Cannot remove stock from empty warehouse') {
        return res.status(400).json({ error: 'Không thể xuất hàng từ kho trống' });
      }
  ...
  async getProductStock(req, res, next) {
    try {
      ...
      const stock = await stockService.getProductTotalStock(productId);
  ...
  async createWarehouse(req, res, next) {
    try {
      ...
      if (!name || !code) {
        return res.status(400).json({ 
          error: 'Tên và mã kho là bắt buộc' 
        });
      }
  ...
  async deleteMovement(req, res, next) {
    try {
      ...
      const result = await stockService.deleteStockMovement(id);
      ...
      if (error.message === 'Stock movement not found') {
        return res.status(404).json({ error: 'Không tìm thấy bản ghi xuất nhập kho' });
      }
      next(error);
    }
  }
