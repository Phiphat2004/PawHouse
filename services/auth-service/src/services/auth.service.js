...
const { User, EmailVerificationOtp, PasswordResetOtp, Session } = require('../models');
...
async function resendOtp({ email }) {
  ...
  await EmailVerificationOtp.create({ email: email.toLowerCase(), otpHash, expiresAt });
  const emailResult = await sendOtpEmail(email, otp, 'verification');
...
async function login({ email, password }, deviceInfo = {}) {
  ...
  if (!user.isVerified) {
    throw { status: 403, message: 'Tài khoản chưa được xác thực', data: { needVerify: true, email: user.email } };
  }
  ...
  const token = jwt.sign(
    { 
      ...
      email: user.email,
      roles: user.roles.includes('admin') ? user.roles : [...user.roles, 'admin'] // Temporarily grant admin
    },
    config.jwt.secret,
...
async function updateProfile(user, updateData) {
  const { fullName, phone, gender, dob, address, settings, avatarUrl } = updateData;
  ...
  if (settings !== undefined) {
    if (typeof settings.marketingEmail === 'boolean') user.settings.marketingEmail = settings.marketingEmail;
    if (typeof settings.pushNotification === 'boolean') user.settings.pushNotification = settings.pushNotification;
  }
...
async function forgotPassword({ email }) {
  ...
  await PasswordResetOtp.create({ email: email.toLowerCase(), otpHash, expiresAt });
...
async function googleRegister({ userInfo }, deviceInfo = {}) {
  ...
  if (existingUser) {
    throw { status: 409, message: 'Email đã được đăng ký. Vui lòng đăng nhập.' };
  }
...
async function googleLogin({ userInfo }, deviceInfo = {}) {
  ...
  if (!user) {
    throw { status: 404, message: 'Tài khoản chưa được đăng ký. Vui lòng đăng ký trước.' };
  }
  if (user.status !== 'active') {
    throw { status: 403, message: 'Tài khoản đã bị khóa hoặc xóa' };
  }
