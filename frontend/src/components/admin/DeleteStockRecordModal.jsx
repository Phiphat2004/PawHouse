...
/**
 * DeleteStockRecordModal - Modal xác nhận xóa stock movement record
 * 
 * Props:
 * - isOpen: boolean - Hiển thị modal
 * - onClose: function - Đóng modal
 * - movement: object - Stock movement cần xóa
 * - onSuccess: function - Callback sau khi xóa thành công
 */
export default function DeleteStockRecordModal({ isOpen, onClose, movement, onSuccess }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      // Success toast
      toast.success('Xóa bản ghi xuất nhập kho thành công!', {
        position: 'top-right',
        autoClose: 3000,
      });
      ...
      const errorMessage = error.message || 'Có lỗi xảy ra khi xóa bản ghi';
